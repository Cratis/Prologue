...
/// <summary>
/// Represents the metadata of a single state-changing HTTP command (<c language="csharp">POST</c>, <c language="csharp">PUT</c>, or <c language="csharp">DELETE</c>)
...
/// <param name="TraceId">The W3C trace id from the request's <c language="csharp">traceparent</c> header, used to correlate the command with the telemetry it produces; empty when absent.</param>
